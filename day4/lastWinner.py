...
def drawNumbers(order, boards):
    for num in order:
        # first mark off number in all boards
        for board in boards:
            for row in board:
                if num in row:
                    # update number in board to indicate that it has been found
                    row[row.index(num)] = '-1'
        # second remove any winning boards
        for board in boards:
            if hasWin(board):
                ...
                if len(boards) == 1:
                    ...
                    return sum, num
                # otherwise more boards remaining, so remove that board
                else:
                    boards.remove(board)
...
if __name__ == "__main__":
    ...
    if sum != 0 and num != 0:
        print("Last winning number:", num)
        print("Last total remaining:", sum)
